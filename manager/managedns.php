    ...
    <div class="container" style="max-width:97% !important;">
       ...
       if(isset($_POST['changestatus'])){
        ...
        if($status==0){
           ...
           $update="UPDATE  dnsrecord  SET  status='$newstatus'  WHERE id='$id' LIMIT 1  ";
           ...
           header("Location: managedns.php?msg=1");
  ...
  <div class="row py-5">
    <div class="col-12">
      ...
      <table id="example" class="table table-hover responsive nowrap" style="width:100%">
        ...
        $selectquesry="SELECT * FROM dnsrecord ";
        
         if(isset($_POST['filtersearch']) ){
               $postst=$_POST['filter'];
               
               if($postst !=3)
               $selectquesry="SELECT * FROM dnsrecord WHERE status='$postst' ";
    ...
    for($i=0;$i<count($data['host']);$i++){
        $tablerows.="<tr> <td> ".$data['host'][$i]."  </td><td> ".$data['type'][$i]." </td>    <td> ".$data['record'][$i]." </td></tr>";
